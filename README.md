...
- 사각형 선 종류와 채우기 투명도, 화살표, 연필, 텍스트, 이미지 삽입과 실행 취소/다시 실행을 제공하는 비파괴 주석 편집
- 클립보드 이미지·텍스트·엑셀 표를 항상 위에 보이는 창으로 고정하고 확대, 투명도, 클릭 통과, 복사, OCR 수행
...
- 최대 20초 구간을 애니메이션 GIF로 내보내며 표준(960px/10fps), 작게(640px/10fps), 최소 용량(480px/5fps) 품질 선택
- 영상 편집기의 내보내기 화면에서 MP4/GIF 형식 선택, 이미지·MP4 절감 목표와 계산된 실제 파일 크기 확인
- 앱 시작 준비 알림과 `MyCapture-{버전}` 창 제목, 이전 캡처 편집기에서 영역 캡처 단축키를 다시 눌러 재촬영
- 캡처 수와 저장 용량을 함께 제한하는 로컬 큐, 이미지의 7일 자동 보관 정리, 썸네일, 빠른 저장 폴더 설정
- 시스템 애니메이션 설정을 존중하는 짧고 일관된 전환과 키보드·스크린 리더 접근성

녹화는 화면 설명·편집·GIF 워크플로에 집중하며 마이크/시스템 오디오는 의도적으로 포함하지 않습니다. 설정에서 한국어·영어 또는 Windows 표시 언어 따르기와 테마(미드나잇 / 데이라이트 / 고대비)를 선택할 수 있습니다. 언어는 다음 앱 시작부터 적용되고, 테마는 적용 즉시 색이 바뀝니다. 구현 범위와 검증 기록은 [`docs/`](docs/)에서 확인할 수 있습니다.
...
| 클립보드 이미지를 GitHub 첨부 URL로 | `F4` | 설정한 이슈 Write 칸에 붙여 넣고 user-attachments URL을 복사 |
...
- 설정, 인덱스, 로그: `%APPDATA%\MyCapture`
- 라이브러리 관리 사본: `%APPDATA%\MyCapture\captures`
...
2.1.0의 변경 사항은 [릴리스 노트](docs/releases/2.1.0-release-notes.md)에서 확인할 수 있습니다. 이전 [2.0.0](docs/releases/2.0.0-release-notes.md), [1.9.2](docs/releases/1.9.2-release-notes.md), [1.9.0](docs/releases/1.9.0-release-notes.md) 기록도 보존합니다.
