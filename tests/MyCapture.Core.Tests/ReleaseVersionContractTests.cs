...
/// <summary>
/// Keeps the 2.3.0 stable version and its Windows binary version aligned across MSBuild, packaging
...
public sealed class ReleaseVersionContractTests
{
    ...
    public void BuildDefaults_AreTheStableOnePointEightPatchThreeBinaryVersion()
    {
        ...
        Assert.Equal("2.3.0", Assert.Single(props.Descendants("Version")).Value);
        Assert.Equal("2.3.0.0", Assert.Single(props.Descendants("FileVersion")).Value);
        Assert.Equal("2.3.0.0", Assert.Single(props.Descendants("AssemblyVersion")).Value);

        XElement identity = Assert.Single(
            manifest.Descendants(),
            element => element.Name.LocalName == "assemblyIdentity");
        Assert.Equal("2.3.0.0", identity.Attribute("version")?.Value);
    }
    ...
    public void PackageAndInstaller_DefaultToTheGaVersion_AndStillAcceptAPrerelease()
    {
        ...
        Assert.Contains("[string]$Version = '2.3.0'", package, StringComparison.Ordinal);
        Assert.Contains("[string]$Version = '2.3.0',", hostile, StringComparison.Ordinal);
