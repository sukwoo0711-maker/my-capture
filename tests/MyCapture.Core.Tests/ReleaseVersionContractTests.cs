...
/// <summary>
/// Keeps the 2.3.4 stable version and its Windows binary version aligned across MSBuild, packaging
...
public sealed class ReleaseVersionContractTests
{
    ...
    public void BuildDefaults_AreTheStableOnePointEightPatchThreeBinaryVersion()
    {
        ...
        Assert.Equal("2.3.4", Assert.Single(props.Descendants("Version")).Value);
        Assert.Equal("2.3.4.0", Assert.Single(props.Descendants("FileVersion")).Value);
        Assert.Equal("2.3.4.0", Assert.Single(props.Descendants("AssemblyVersion")).Value);
    ...
    public void PackageAndInstaller_DefaultToTheGaVersion_AndStillAcceptAPrerelease()
    {
        ...
        Assert.Contains("[string]$Version = '2.3.4'", package, StringComparison.Ordinal);
        Assert.Contains("[string]$Version = '2.3.4',", hostile, StringComparison.Ordinal);
